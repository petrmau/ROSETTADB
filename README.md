...
6. [Downloading sources](#downloading-sources)
...
8. [Export scripts](#export-scripts)
9. [ingest.py flags](#ingestpy-flags)
10. [protein_cluster.py flags](#protein_clusterpy-flags)
...
| NCBI AMRFinderPlus | ~8 302 sequences | Gene family, class, subclass, scope, type; source files: `AMR_CDS.fa` + `ReferenceGeneCatalog.txt` from NCBI AMRFinder FTP |
| ResFinder | ~3 212 sequences | Drug class, phenotype, PMID, required co-gene |

Use `download.sh` to fetch or update all three sources (see [Downloading sources](#downloading-sources)).
...
amr.sequence_drug_class – one row per unique sequence × canonical drug class
                          Evidence paths:
                            ...
                            ResFinder: gene.drug_class text → drug_class.resfinder_alias
                                       + gene.aro_accession (from argNorm) → card_gene_class → drug_class
...
| `argnorm_aro_map.tsv` | ~16 376 | argNorm 1.1.0 ARO enrichment map (3 067 ResFinder + 13 309 NCBI keys); keyed by full FASTA header (ResFinder) or allele / protein accession (NCBI); only Perfect / Strict / Manual cut-off entries included; **regenerated by `harmonise.py`** when argNorm is installed |
...
| `harmonise.py` | Parses all three AMR sources + CARD ARO OBO and produces `drug_canonical.tsv`, `drug_alias.tsv`, `drug_class_member.tsv`, `gene_drug_link.tsv`, `aro_drug_class_member.tsv`, `aro_gene_class.tsv`, `aro_gene_drug.tsv`; also calls `build_argnorm_aro_map()` (requires `argnorm` package) to regenerate `argnorm_aro_map.tsv`; loads `antimicrobials.txt` at startup to build a synonym→INN map (see *INN normalisation* below); assigns `context` flags via `_context_flag()` — edit the `CLASS_TERM_DRUGS` set here to manage class-level token entries |
...
#           Also regenerates argnorm_aro_map.tsv (requires: pip install argnorm)
...
`download.sh` fetches or updates all three primary AMR source databases.

```bash
./download.sh              # update all three sources
./download.sh card         # CARD only
./download.sh resfinder    # ResFinder only
./download.sh amrfinder    # AMRFinder only
```

| Source | Method | Target directory |
|--------|--------|-----------------|
| CARD | `wget` from `card.mcmaster.ca/latest/data` (`.tar.bz2`) | `sources/CARD/` |
| ResFinder | `git clone` / `git pull` from Bitbucket | `sources/resfinder_db/` |
| AMRFinder | `wget` five files from NCBI FTP `latest/`; MD5 checksums verified | `sources/amr_finder_plus/` |

AMRFinder files downloaded: `AMR_CDS.fa`, `ReferenceGeneCatalog.txt`, `AMRProt.fa`, `fam.tsv`, `md5sum.txt`.

Each source writes a `VERSION` (or `ROSETTA_VERSION` for ResFinder) file recording the download timestamp and version. The script exits with a non-zero status if any AMRFinder checksum fails.
...
```bash
pip install psycopg2-binary argnorm
```
...
- Enriches ResFinder and NCBI genes with ARO accessions from `harmonise/argnorm_aro_map.tsv`
...
| `--ncbi` | `sources/amr_finder_plus/AMR_CDS.fa` | NCBI FASTA path (AMRFinder FTP format) |
